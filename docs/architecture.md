...
**Default composition (as of the Phase 10 poster-design rebuild):** `buildDefaultLayout` recreates the visual composition of the league's previous (pre-application) fixture creator rather than a generic stack of equal-weight boxes: a dark masthead (an optional supplied photo/graphic is shown behind a diagonal clip on its lead edge; with none, a strong dark fallback - never a blank rectangle), a three-band brand trim immediately beneath it (always the season's own primary/secondary/accent colours, with no per-band override), one bordered block holding all division rows with light dividers between them, a dark-headed fixture matrix, a borderless "dotted leader" competitions strip, and a rules box with a certificate-style label straddling its top border. `GlobalStyle` gained a `secondaryColor` field for the trim's middle band, and `HeaderElement` gained an optional `backgroundImage`, both additive/backward-compatible with layouts saved before this change.
